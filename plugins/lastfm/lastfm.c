...
lfm_update_auth (void) {
    deadbeef->conf_lock ();
    const char *user = deadbeef->conf_get_str_fast ("lastfm2.secret.login", "");
    const char *pass = deadbeef->conf_get_str_fast ("lastfm2.secret.password", "");
...
lfm_message (uint32_t id, uintptr_t ctx, uint32_t p1, uint32_t p2) {
    ...
    case DB_EV_CONFIGCHANGED:
        if (deadbeef->conf_get_int ("lastfm2.trace", 0)) {
...
_migrate_secrets(void) {
    const char *user = deadbeef->conf_get_str_fast ("lastfm2.secret.login", NULL);
    const char *pass = deadbeef->conf_get_str_fast ("lastfm2.secret.password", NULL);

    if (user == NULL) {
        user = deadbeef->conf_get_str_fast ("lastfm2.login", NULL);
        if (user != NULL) {
            deadbeef->conf_set_str("lastfm2.secret.login", user);
            deadbeef->conf_remove_items("lastfm2.login");
        }
    }

    if (pass == NULL) {
        pass = deadbeef->conf_get_str_fast ("lastfm2.password", NULL);
        if (pass != NULL) {
            deadbeef->conf_set_str("lastfm2.secret.password", pass);
            deadbeef->conf_remove_items("lastfm2.password");
        }
    }
...
static const char settings_dlg[] =
    ...
    "property Username entry lastfm2.secret.login \"\";\n"
    "property Password password lastfm2.secret.password \"\";"
    ...
    "property \"Enable logging\" checkbox lastfm2.trace 0;\n";
;
